...
TotalSims=1 # batch size : How many simulations to accumulate before update the neuron network
TotalBatch=10000 # number of batch to calculate
option = '8075x' # resize the figure 75X80 options are '4040', '7580','8080'
...
while batchnum < TotalBatch:
    ...
    while not (q.empty() ):
        q.get()
    if not finish: # if the game was not simulated correct, abandon this simulation and continue
        continue
