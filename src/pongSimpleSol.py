...
option = '4040' # resize the figure 75X80 options are '4040', '7580','8080'
gameoption = 'rgb_array' # gameoptions are 'rgb_array', 'rot90', 'rot90down14'
...
if resume:
    model = pickle.load(open('save.pshort', 'rb'))
    timelist =         np.load( "./pongSimpleSol/"+"/timelist.npy")
    batchlist =      np.load( "./pongSimpleSol/"+"/batchlist.npy")
    rewardlist =      np.load( "./pongSimpleSol/"+"/rewardlist.npy")
else:
    model = {}
...
while batchnum < TotalBatch:
    ...
    xs, drs, dlogps, hs = psfunc.worker(env, q, model, NSims=TotalSims)
    while not (q.empty() ):
        print (q.empty())
        q.get()
...
#    print ('resetting env. episode reward total was %f. running mean: %f'%(reward_sum, running_reward) )
 ...
 #   print (batchlist)
    ...
    if episode_number % 10 == 0: #save results
        pickle.dump(model, open('save.pshort', 'wb'))
